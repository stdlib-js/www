...
function validate( opts, options ) {
	...
	if ( hasOwnProp( options, 'address' ) ) {
		...
		if ( !isString( opts.address ) ) {
			return new TypeError( 'invalid option. `address` option must be a string. Option: `' + opts.address + '`.' );
		}
	}
	...
	if ( hasOwnProp( options, 'latest' ) ) {
		...
		if ( !isString( opts.latest ) ) {
			return new TypeError( 'invalid option. `latest` option must be a string. Option: `' + opts.latest + '`.' );
		}
	}
	if ( hasOwnProp( options, 'logger' ) ) {
		...
		if ( isString( opts.logger ) ) {
			...
			return new TypeError( 'invalid option. `logger` option must be either a boolean or a string. Option: `' + opts.logger + '`.' );
		}
	}
	...
	if ( hasOwnProp( options, 'prefix' ) ) {
		...
		if ( !isString( opts.prefix ) && !isStringArray( opts.prefix ) ) {
			return new TypeError( 'invalid option. `prefix` option must be either a string or an array of strings. Option: `' + opts.prefix + '`.' );
		}
	}
	if ( hasOwnProp( options, 'root' ) ) {
		opts.root = options.root;
		if ( !isString( opts.root ) ) {
			return new TypeError( 'invalid option. `root` option must be a string. Option: `' + opts.root + '`.' );
		}
	}
	if ( hasOwnProp( options, 'static' ) ) {
		opts.static = options.static;
		if ( !isString( opts.static ) && !isStringArray( opts.static ) ) {
			return new TypeError( 'invalid option. `static` option must be either a string or an array of strings. Option: `' + opts.static + '`.' );
		}
	}
	if ( hasOwnProp( options, 'template' ) ) {
		opts.template = options.template;
		if ( !isString( opts.template ) ) {
			return new TypeError( 'invalid option. `template` option must be a string. Option: `' + opts.template + '`.' );
		}
	}
	if ( hasOwnProp( options, 'app' ) ) {
		opts.app = options.app;
	}
	if ( hasOwnProp( options, 'trustProxy' ) ) {
		opts.trustProxy = options.trustProxy;
		if ( !isBoolean( opts.trustProxy ) ) {
			return new TypeError( 'invalid option. `trustProxy` option must be a boolean. Option: `' + opts.trustProxy + '`.' );
		}
	}
	if ( hasOwnProp( options, 'ignoreTrailingSlash' ) ) {
		...
		if ( !isBoolean( opts.ignoreTrailingSlash ) ) {
			return new TypeError( 'invalid option. `ignoreTrailingSlash` option must be a boolean. Option: `' + opts.ignoreTrailingSlash + '`.' );
		}
	}
	return null;
}
