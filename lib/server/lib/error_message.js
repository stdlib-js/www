...
function errorMessage( code, args ) {
	...
	if ( !pkg ) {
		return '(invalid error code. Cannot display error message. Unable to resolve the package from which error originated based on the provided error code.)';
	}
	msg = id2msg( code.substring( 3 ) );
	if ( !msg ) {
		return '(invalid error code. Cannot display error message. Unable to resolve an error message associated with the provided error code.)';
	}
	tmp =[ msg ];
	for ( i = 0; i < args.length; i++ ) {
		tmp.push( args[ i ] );
	}
	return format.apply( null, tmp );
}
