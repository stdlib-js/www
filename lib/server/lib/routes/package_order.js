...
/**
* Defines a route handler for returning package order for a requested documentation version.
*
* @private
...
function route( opts ) {
	var schema = {
		'method': 'GET',
		'url': '/docs/api/:version/package_order.json',
		'schema': {
			'response': {
				'200': {
					'type': 'string'
	...
	*/
	function onRequest( request, reply ) {
		...
		p = resolve( opts.root, v, 'package_order.json' );
