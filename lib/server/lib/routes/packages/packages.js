	...
	*/
	function onResults( error, results ) {
		...
		if ( error ) {
			...
			// We assume that the `ENOENT` (i.e., no "entry" (file or directory) error) indicates a missing package README; in which case, we want the application to "404", rather than return a JSON error, so we simply SSR an empty application shell and allow client side rendering to generate the 404 page...
			file = '';
			data = {};
		} else {
			file = results[ 0 ];
		...
		html = render( sheets.collect( app( self.App, url, v, file, data, request.locals, {} ) ) );  // eslint-disable-line max-len
